...
export default function EditClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function loadCurrentSub() {
    const { data: subs } = await supabase
      ...
      .select("id, start_date, end_date, status, extended_days, extension_reason, plans(name)")
      .eq("client_id", id)
    ...
    if (subs && subs.length > 0) {
      ...
      const s = subs[0] as unknown as { id: string; start_date: string; end_date: string; status: string; extended_days: number; extension_reason: string | null; plans: { name: string } | null };
      setCurrentSub({ id: s.id, plan_name: s.plans?.name ?? "—", start_date: s.start_date, end_date: s.end_date, status: s.status, extended_days: s.extended_days ?? 0, extension_reason: s.extension_reason });
    }
  }
      ...
      <div className="border border-neutral-200 rounded-lg p-4 mb-6 bg-neutral-50">
        ...
        {hasAnySubscription && currentSub ? (
          <div>
            ...
            {currentSub.extended_days > 0 && (
              <p className="text-xs text-neutral-400 mt-0.5">Extended by {currentSub.extended_days} day(s){currentSub.extension_reason ? ` — ${currentSub.extension_reason}` : ""}</p>
            )}
          </div>
        ) : (
