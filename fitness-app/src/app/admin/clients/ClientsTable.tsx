...
export type ClientRow = {
  ...
  current_subscription: { status: "active" | "expired" | "expiring" | "none"; end_date: string | null; days_left: number | null } | null;
};
...
const SUB_STATUS_META: Record<string, { label: (daysLeft: number | null) => string; badge: string }> = {
  active: { label: () => "Active", badge: "bg-emerald-50 text-emerald-700" },
  expiring: { label: (d) => `Expiring in ${d}d`, badge: "bg-amber-50 text-amber-700" },
  expired: { label: () => "Expired", badge: "bg-rose-50 text-rose-600" },
  none: { label: () => "No subscription", badge: "bg-neutral-100 text-neutral-400" },
};
...
export default function ClientsTable({
  ...
  clients: ClientRow[];
  onOpenSubscription: (clientId: string) => void;
}) {
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b border-neutral-200 bg-neutral-50">
                ...
                <th className="text-left font-medium text-neutral-500 px-4 py-2.5 whitespace-nowrap">Subscription</th>
                <th className="text-left font-medium text-neutral-500 px-4 py-2.5 whitespace-nowrap">Status</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((c) => {
                const sub = c.current_subscription ?? { status: "none" as const, end_date: null, days_left: null };
                ...
                return (
                  <tr key={c.id} className="border-b border-neutral-100 last:border-0 hover:bg-rose-50/40">
                    ...
                    <td className="px-4 py-2.5 whitespace-nowrap">
                      <button onClick={() => onOpenSubscription(c.id)} className="text-left">
                        <span className={`text-xs px-2 py-0.5 rounded-full ${meta.badge}`}>
                          {meta.label(sub.days_left)}
                        </span>
                        {sub.end_date && <div className="text-[11px] text-neutral-400 mt-0.5">till {sub.end_date}</div>}
                      </button>
                    </td>
                    <td className="px-4 py-2.5 whitespace-nowrap">
