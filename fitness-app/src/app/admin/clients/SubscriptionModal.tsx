...
export default function SubscriptionModal({
  ...
  const computedEndDate = selectedPlan && startDate ? addDays(startDate, selectedPlan.duration_days) : null;
  ...
  async function loadHistory(clientId: string) {
    ...
    const { data } = await supabase
      ...
      .select("id, start_date, end_date, status, payment_amount, payment_mode, payment_date, extended_days, plans(name)")
      .eq("client_id", clientId)
      .order("start_date", { ascending: false });

    const rows = (data ?? []).map((s) => {
      const row = s as unknown as { id: string; start_date: string; end_date: string; status: string; payment_amount: number | null; payment_mode: string | null; payment_date: string | null; extended_days: number; plans: { name: string } | null };
      return { id: row.id, plan_name: row.plans?.name ?? "—", start_date: row.start_date, end_date: row.end_date, status: row.status, payment_amount: row.payment_amount, payment_mode: row.payment_mode, payment_date: row.payment_date, extended_days: row.extended_days ?? 0 };
    });
    setHistory(rows);
  ...
  async function handleExtend(row: SubRow) {
    ...
    await supabase.from("client_subscriptions").update({
      end_date: addDays(row.end_date, days),
      ...
      <div className="bg-white rounded-xl p-5 max-w-lg w-full shadow-lg max-h-[85vh] overflow-y-auto">
        ...
        {selectedClientId && (
          <>
              ...
              <div className="flex flex-col gap-3 mb-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input type="text" readOnly value={computedEndDate ?? ""} className="w-full rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-2 text-sm text-neutral-500" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                  ...
                  <div>
                    <label className="block text-xs text-neutral-500 mb-1">Payment date</label>
                    <input type="date" value={paymentDate} onChange={(e) => setPaymentDate(e.target.value)} className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm" />
                  </div>
                </div>
                <div>
            ...
            {loadingHistory ? (
              <p className="text-xs text-neutral-400 flex items-center gap-2"><Spinner className="w-3 h-3" /> Loading...</p>
            ) : history.length === 0 ? (
              <p className="text-xs text-neutral-400">No subscriptions yet for this client.</p>
            ) : (
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-neutral-50 border-b border-neutral-200">
                      ...
                      <th className="text-left font-medium text-neutral-500 px-3 py-2">Start</th>
                      <th className="text-left font-medium text-neutral-500 px-3 py-2">End</th>
                    ...
                    {history.map((row, idx) => (
                      <tr key={row.id} className="border-b border-neutral-100 last:border-0 align-top">
                        ...
                        <td className="px-3 py-2 whitespace-nowrap">
                          ...
                          {row.payment_mode && <div className="text-neutral-400">{row.payment_mode}{row.payment_date ? ` · ${row.payment_date}` : ""}</div>}
                        </td>
                        <td className="px-3 py-2 text-right">
                          {idx === 0 && extendId !== row.id && (
